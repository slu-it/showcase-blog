...
describe('BlogPostPreview', () => {
  ...
  describe('rendering', () => {
    ...
    it('should display the formatted publication time', () => {
      // Use Angular's DatePipe with the same locale and format as the template
      // to derive the expected value, so the assertion is timezone-agnostic.
      const expected = new DatePipe('en-UK')
